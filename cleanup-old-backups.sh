...
cleanup_old_backups() {
    ...
    # 古いログファイルを削除
    find "$BACKUP_DIR" -name "backup_log_*.txt" -type f -mtime +${days} -delete 2>/dev/null || true
    find "$BACKUP_DIR" -name "cleanup_log_*.txt" -type f -mtime +${days} -delete 2>/dev/null || true
    
    # 古い一時ディレクトリを削除（万が一残っている場合）
    find "$BACKUP_DIR" -name "temp_*" -type d -mtime +1 -exec rm -rf {} + 2>/dev/null || true
